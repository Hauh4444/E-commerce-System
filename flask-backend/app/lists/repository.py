...
class ListsRepository:
    ...
    def get_list_by_id(self, user_id: str, list_id: str):
        ...
        if not lookup_id:
            return None
        return self.lists.find_one({"_id": lookup_id, "user_id": user_id})
    ...
    def update_list(self, user_id: str, list_id: str, updates: dict):
        ...
        result = self.lists.update_one({"_id": lookup_id, "user_id": user_id}, {"$set": updates})
        if result.matched_count == 0:
            return None
        return self.lists.find_one({"_id": lookup_id})

    def add_product(self, user_id: str, list_id: str, product_id: str):
        lookup_id = parse_object_id(list_id)
        product_obj_id = parse_object_id(product_id)
        if not lookup_id or not product_obj_id:
            return None
        result = self.lists.update_one(
            {"_id": lookup_id, "user_id": user_id},
            {"$addToSet": {"product_ids": product_obj_id}, "$set": {"updated_at": datetime.now()}}
        )
        if result.matched_count == 0:
            return None
        return self.lists.find_one({"_id": lookup_id})
    ...
    def remove_product(self, user_id: str, list_id: str, product_id: str):
        ...
        if result.matched_count == 0:
            return None, False
        return self.lists.find_one({"_id": lookup_id}), result.modified_count > 0

    def delete_list(self, user_id: str, list_id: str):
        lookup_id = parse_object_id(list_id)
        if not lookup_id:
            return False
        result = self.lists.delete_one({"_id": lookup_id, "user_id": user_id})
